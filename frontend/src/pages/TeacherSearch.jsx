...
export default function TeacherSearch() {
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(async () => {
      ...
      try {
        ...
        const result = trimmed ? await searchTeachers(trimmed) : await getTeachers();
        setTeachers(result);
      } catch (requestError) {
        setTeachers([]);
  ...
  return (
    <section className="search-layout">
          ...
          {mode === MODES.name ? (
            ...
            <div className="time-search-grid">
              <label
                ...
                onClick={() => daySelectRef.current?.focus()}
              >
                <span className="search-icon">◫</span>
                <select
                  ref={daySelectRef}
      ...
      {mode === MODES.name && name.trim() === "" && !loading && !error ? null : null}
